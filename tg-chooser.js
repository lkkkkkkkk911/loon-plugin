...
function buildLinks(url) {
  ...
  if (path.startsWith("+")) {
    ...
    swtf = `sg://join?invite=${encodeURIComponent(invite)}`;
    ...
    swtf = `sg://privatepost?channel=${encodeURIComponent(channel)}&post=${encodeURIComponent(post)}`;
    ...
    swtf = `sg://resolve?${query}`;
    nice = `nicegram://resolve?${query}`;
  }

  const web = rawUrl + (rawUrl.includes("?") ? "&" : "?") + "tgchooser=web";
  return { tg, sw, swtf, nice, web };
}
...
<style>
body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#f2f2f7;padding:28px}
.card{background:white;border-radius:22px;padding:24px}
h2{margin-top:0}
a{display:block;padding:16px;margin:14px 0;border-radius:14px;text-align:center;text-decoration:none;color:white;font-size:18px}
.tg{background:#229ED9}
.sw{background:#ff6b35}
.swtf{background:#111}
.nice{background:#5865f2}
.web{background:#555}
.url{margin-top:18px;font-size:13px;color:#777;word-break:break-all}
</style>
</head>
<body>
<div class="card">
...
<a class="sw" href="${links.sw}">Swiftgram 正式版</a>
<a class="swtf" href="${links.swtf}">Swiftgram TF 版</a>
